...
\institute{Rice University$^1$ and Tampere University of Technology$^2$} % See Section 4.1
...
\renewcommand\TP@maketitle{%
  \centering
   \begin{minipage}[b]{1\linewidth}
        ...
        {\LARGE \@institute}
    \end{minipage}%
%      \tikz[overlay]\node[scale=1,anchor=east,xshift=16in,yshift=0.9in,inner sep=0pt] {%
%       \includegraphics[width=4in]{tut.jpg}
 %         }; 
      \tikz[overlay]\node[scale=1,anchor=east,xshift=4.4in,yshift=1.3in,inner sep=0pt] {%
       \includegraphics[width=4in]{Rice_logo_hi_res.png}       
   };   
}
\makeatother
...
\begin{itemize}
    \item {\bf Learn DPD coefficient, $\alpha$, for each spur}
        \begin{itemize}
...
\column{0.33}
\block{Mathematical Model}{\Large
\begin{itemize}
    \item {\bf PA Inputs and Outputs:}
    \begin{itemize}
        \item Parallel Hamerstein baseband PA model
        \item Third order, memoryless
    \end{itemize}
\begin{align}
\text{\bf Input:} \ x(n) =& \sum_{i = 1}^N x_i(n) e^{2\pi j n \frac{f_i}{f_s}}, \label{eq:PA_In} \\
\text{\bf Output:} \ y(n) =& \beta_1 x(n) + \beta_3 |x(n)|^2 x(n). \label{eq:PA}
\end{align}
\vspace{1mm}
\item {\bf IM3 Spurious Signals and Their Locations}
\begin{align}
y_{IM3:i;j,k}(n) &= x_i^*(n)x_j(n)x_k(n), \label{eq:IM3_out} \\
f_{IM3:i;j,k} &= -f_i + f_j + f_k.
\end{align}
\item {\bf DPD Processing}
\begin{align}
\tilde{x}(n) &= 
\sum_{i=1}^N x_i(n) e^{2\pi j n \frac{f_i}{f_s} n}  \nonumber\\ 
&+ \sum_{i = 1}^{N} \sum_{j = 1}^N \sum_{k = 1}^N \alpha_{i;j,k}  \ x_i^*(n) x_j(n) x_k(n) e^{2\pi j n\frac{-f_i + f_j + f_k}{f_s}}.
\label{eq:PA_In_dpd}
\end{align}
\end{itemize}
}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Three, 20MHz CCs, scenario CA 41C-41A
    ...
    \item {\bf Result}
    \begin{itemize}
        \item Rapid convergence of coefficients
        \item Suppression below the emission limit in the spurious region
    \end{itemize}
\end{itemize}
%\begin{tikzfigure}[]
%\label{fig:fig1}
...
\column{0.33}
\block{DPD Coefficient Learning}{\Large
%\begin{tikzfigure}
%\includegraphics[width=0.97\linewidth]{SystemArchi}
%\end{tikzfigure}
\vspace{3mm}
\begin{itemize}
    \item {\bf LMS Adaptive Training}
        \vspace{2mm}
           \begin{align} 
%    \boldsymbol{\alpha}(m) &= [\alpha_0(m) \:\: \alpha_2(m) \:\: ... \:\: \alpha_N(m)]^T \\
    \alpha_{i;j,k}(n+1) &= \alpha_{i;j,k}(n) -\mu \frac{ x_i^*(n) x_j(n) x_k(n){e}^*_{i;j,k}(n)}{|| x_i^*(n) x_j(n) x_k(n)||}
%    \label{eq:NLMS_Update_memory}
    \end{align}
    \vspace{1mm}
    \begin{itemize}
        \item Decorrelates the error signal, $e$, with basis functions from Equation 3 to learn the DPD coefficients, $\alpha$.
        \item Rate is controlled by LMS parameter $\mu$.
        \item Learning is resilient to temperature changes, PA Gain changes, and changes in signal characteristics.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    %\end{itemize}
    \item {\bf Hardware testing on the USRP with external PAs }
    %\begin{itemize}
